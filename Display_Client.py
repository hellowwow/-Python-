...
def Click_Send():
    ...
    if radio_var.get() == 1:
        ...
        if RawSocket_send.ip_id_hide_send(message.encode('gbk'), srcip.get(), dstip.get(), int(srcport.get()), int(dstport.get())):
...
but1 = tkinter.Button(win, text='Send_Message:', font=ft, relief='flat', width=30, height=1, fg='blue', state="disabled", anchor=tkinter.W)
...
but2 = tkinter.Button(win, text='Recv_Message:', font=ft, relief='flat', width=30, height=1, fg='blue', state="disabled", anchor=tkinter.W)
